...
const (
	continuePush = "Yes. Push the branches to GitLab."
	abortPush    = "No. Do not push the branches to GitLab."

	reasonAlreadyUpToDate   = "Already up-to-date."
	reasonNotPushedToRemote = "No remote branch yet."
	reasonMRIsMerged        = "MR is already merged."
	reasonMRIsClosed        = "MR is closed."
	reasonParentNotPushed   = "Parent branch is not pushed to remote."
	reasonNoMR              = "Some branches in a stack do not have a MR."
)
...
type pushCandidate struct {
	...
	remoteMRMeta actions.PRMetadata
	localMRMeta  actions.PRMetadata
}
...
func NewGitLabPushModel(
	...
	if avconfig.Av.PullRequest.RebaseWithDraft != nil {
		...
		// TODO: Implement GitLab-specific CODEOWNERS detection
		makeDraftBeforePush = false
	}
	return &GitLabPushModel{
		repo:                repo,
		db:                  db,
		client:              client,
		...
		mergeRequestsCache:  map[string]*gl.MergeRequest{},
	}
}

type GitLabPushProgress struct {
	candidateCalculationDone bool
	gitPushDone              bool
}

type GitLabPushDone struct{}

type GitLabPushModel struct {
	repo                *git.Repo
	db                  meta.DB
	client              *gl.Client
	...
	// key is internal/gl/mergerequest.MergeRequest.ID
	mergeRequestsCache map[string]*gl.MergeRequest
...
func (vm *GitLabPushModel) View() string {
	...
	if len(vm.pushCandidates) == 0 {
		sb.WriteString(colors.SuccessStyle.Render("✓ Nothing to push to GitLab"))
	} else if vm.askingForConfirmation {
		sb.WriteString("Confirming the push to GitLab")
	} else if vm.runningGitPush {
		sb.WriteString(colors.ProgressStyle.Render(vm.spinner.View() + "Pushing to GitLab..."))
	} else if vm.done {
		if vm.chooseNoPush {
			sb.WriteString(colors.SuccessStyle.Render("✓ Not pushing to GitLab"))
		} else {
			sb.WriteString(colors.SuccessStyle.Render("✓ Pushed to GitLab"))
		}
	}
...
func (vm *GitLabPushModel) viewPushCandidates() string {
	sb := strings.Builder{}
	for i, branch := range vm.pushCandidates {
		...
		sb.WriteString("         Parent=" + branch.remoteMRMeta.Parent + "\n")
		sb.WriteString(
		...
		sb.WriteString("         Parent=" + branch.localMRMeta.Parent + "\n")
		avbr, _ := vm.db.ReadTx().Branch(branch.branch.Short())
		if avbr.PullRequest != nil && avbr.PullRequest.Permalink != "" {
			sb.WriteString("  MR:     " + avbr.PullRequest.Permalink + "\n")
		}
	}
	return sb.String()
}

func (vm *GitLabPushModel) viewNoPushBranches() string {
	sb := strings.Builder{}
...
func (vm *GitLabPushModel) runUpdate() (ret tea.Msg) {
	// TODO: Implement GitLab-specific merge request operations
	// This is a placeholder that will be implemented in later steps
	
	if err := vm.runGitPush(); err != nil {
		return err
	}
	return &GitLabPushProgress{gitPushDone: true}
}

func (vm *GitLabPushModel) runGitPush() error {
	...
	if err != nil {
		return errors.WrapIff(err, "failed to push branches to GitLab")
	}
	if res.ExitCode != 0 {
		return errors.Errorf("failed to push branches to GitLab\n%s\n%s", res.Stdout, res.Stderr)
	}
...
func (vm *GitLabPushModel) calculateChangedBranches() tea.Msg {
	...
	for _, br := range vm.targetBranches {
		...
		localMRMeta := vm.createMRMetadata(avbr)
		remoteMRMeta := actions.PRMetadata{}
		// TODO: Implement remote MR metadata reading
		// This would read GitLab merge request metadata from remote MR

		if localRef.Hash() == remoteRef.Hash() && localMRMeta == remoteMRMeta {
			noPushBranches = append(noPushBranches, noPushBranch{
				branch: br,
		...
		if !vm.allBranchesOnStackHaveMRs(br) {
			noPushBranches = append(noPushBranches, noPushBranch{
				branch: br,
				reason: reasonNoMR,
			})
		...
		pushCandidates = append(pushCandidates, pushCandidate{
			...
			remoteMRMeta: remoteMRMeta,
			localMRMeta:  localMRMeta,
		})
	}
	vm.noPushBranches = noPushBranches
	vm.pushCandidates = pushCandidates
	return &GitLabPushProgress{candidateCalculationDone: true}
}

func (vm *GitLabPushModel) allParentsHaveRemoteTrackingBranch(
...
func (vm *GitLabPushModel) allBranchesOnStackHaveMRs(br plumbing.ReferenceName) bool {
	avbr, _ := vm.db.ReadTx().Branch(br.Short())
	for {
...
func (vm *GitLabPushModel) createMRMetadata(branch meta.Branch) actions.PRMetadata {
